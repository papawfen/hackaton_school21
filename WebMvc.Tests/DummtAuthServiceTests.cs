...
public class Tests
{
    ...
    public void TryRegister_RegisterTwoUsers()
    {
        ...
        users.ForEach(user => authService.TryRegister(user).Wait());
    ...
    {
        var authService = new DummyAuthServiceForTests();
        authService.TryRegister(new User("First", "First")).Wait();

        Assert.That(() => authService.TryRegister(new User("First", "First")).Wait(), Throws.Exception);
    }
    ...
    public void TryLogin_RegisterAndLogin_Success()
    {
        ...
        authService.TryRegister(user).Wait();

        Assert.That(() => authService.TryAuthenticate(user), Throws.Nothing);
